...
var SlackAccessor = (function () {
  ...
  p.requestChannelInfo = function () {
    var option = {
      types: "public_channel, private_channel", // プライベートチャンネルも対象に入れる
    };
  ...
  // 特定チャンネルのメッセージ取得
  p.requestMessages = function (channel, oldest) { // 特定チャンネルのメッセージ取得
...
var SpreadsheetController = (function () {
  ...
  p.getLastTimestamp = function (channel, is_reply) {
    ...
    if (lastRow > 0) {
      ...
      let col_time = col_int + (col_double / 1000000);
      return col_time;
    }
    return "1";
  };
